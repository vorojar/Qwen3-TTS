...
| 阶段 | 工具 | 效果 |
|------|------|------|
| 人声提取 | Demucs (Meta) | 去除背景音乐、环境噪音，保留纯净人声 |
| 静音裁切 | Silero VAD | 去除首尾静音，压缩中间过长停顿 |
| 文本正规化 | wetext | 数字/日期/货币转口语形式（如 "100元"→"一百元"） |

**响度标准化**：每句生成音频自动标准化到 **-16 LUFS**（有声书/流媒体行业标准，pyloudnorm），不同说话人、不同模式的音量统一，混合声音不再忽大忽小。

以上均为**可选依赖**，未安装时自动跳过，不影响正常使用。
...
- **批量推理** — 多句合并为一次模型调用（BATCH_SIZE=16），速度提升约 2 倍
- **编辑/结果视图切换** — 在文本编辑和句子结果间自由切换，状态不丢失
...
**可选依赖（推荐）：**
```bash
pip install wetext        # 中文文本正规化（数字/日期→口语形式）
pip install silero-vad    # 克隆参考音频静音裁切
pip install demucs        # 克隆参考音频人声提取（去背景音乐/噪音）
pip install pyloudnorm    # 响度标准化（-16 LUFS，统一音量）
```
...
**性能优化**
- 批量推理：多句合并为一次模型调用（BATCH_SIZE=16），预设模式速度提升约 2 倍
- SDPA 融合注意力：`attn_implementation="sdpa"` 启用 PyTorch 原生融合注意力内核
- GPU 预热：模型加载后进行一次 dummy 推理，避免首次生成的冷启动延迟
- 全部推理入口使用 `torch.inference_mode()` 包装

**音频预处理流水线**
- Demucs (Meta) 人声提取：自动去除克隆参考音频中的背景音乐和环境噪音（80MB 模型，GPU 约 1 秒/10 秒音频）
- Silero VAD 静音裁切：去除参考音频首尾静音，压缩中间过长停顿（超过 300ms 截短）
- wetext 中文文本正规化：数字、日期、货币、百分比自动转口语形式（如 "100元"→"一百元"、"2024年"→"二零二四年"）
- pyloudnorm 响度标准化：每句生成音频标准化到 -16 LUFS，不同声音/模式音量一致
- 四个均为可选依赖，未安装时自动跳过

**界面改进**
- 编辑/结果视图切换：在文本编辑和句子编辑器间自由切换，所有状态保持不变
- 进度模拟：批量推理时平滑逐句高亮（渐近缓出曲线）
...
| 🔲 | 语速/音调控制 | 逐句调整语速和音调 |
| ✅ | 音频预处理 | Demucs 人声提取 + VAD 静音裁切 + 文本正规化 |
| 🔲 | 发音词典 | 人名/地名/术语发音纠正 |
| 🔲 | 实时流式输出 | 依赖上游 SDK 支持 |
